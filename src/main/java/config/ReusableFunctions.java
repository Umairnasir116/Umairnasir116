...
public class ReusableFunctions {
        ...
        return (ArrayList)((ValidatableResponse)EnvGlobals.response.then()).extract().path(key, new String[0]);
        }

        public static int getResponseLength() {
        return (Integer)EnvGlobals.response.body().path("list.size()", new String[0]);
        }

        public static void printResponse() {
        if (EnvGlobals.response != null) {
        ...
        return EnvGlobals.response != null ? EnvGlobals.response.getBody().asString() : null;
        }

        public static int getResponseLengthByKey(String Key) {
        return (Integer)EnvGlobals.response.body().path(Key, new String[0]);
        }
...
//        }
        ...
        EnvGlobals.requestSpecification = REQUEST.given();
        if (headers == null) {
        EnvGlobals.requestSpecification = REQUEST.given().body(payload);
        } else if (payload == null) {
        EnvGlobals.requestSpecification = REQUEST.given().headers(headers);
        } else {
        EnvGlobals.requestSpecification = REQUEST.given().headers(headers).body(payload);
        }
        ...
        EnvGlobals.requestSpecification = REQUEST.given().queryParams(params).headers(headers);
        ...
        switch(var3) {
        case 0:
        EnvGlobals.response = (Response)((RequestSpecification)EnvGlobals.requestSpecification.when().log().all()).post(endPoint, new Object[0]);
        break;
        case 1:
        EnvGlobals.response = (Response)((RequestSpecification)EnvGlobals.requestSpecification.when().log().all()).get(endPoint, new Object[0]);
        break;
        case 2:
        EnvGlobals.response = (Response)((RequestSpecification)EnvGlobals.requestSpecification.when().log().all()).delete(endPoint, new Object[0]);
        break;
        case 3:
        EnvGlobals.response = (Response)((RequestSpecification)EnvGlobals.requestSpecification.when().log().all()).put(endPoint, new Object[0]);
        break;
        case 4:
        EnvGlobals.response = (Response)((RequestSpecification)EnvGlobals.requestSpecification.when().log().all()).patch(endPoint, new Object[0]);
        }

        }

        public static void thenFunction(int statusCode) {
        ((ValidatableResponse)((ValidatableResponse)EnvGlobals.response.then()).log().all()).statusCode(statusCode);
        }

        public static void thenObjectmatch(String path, String matchers) {
        ((ValidatableResponse)EnvGlobals.response.then()).body(path, Matchers.hasItem(matchers), new Object[0]);
        }

        public static void thenEqualMatch(String path, String matchers) {
        ((ValidatableResponse)EnvGlobals.response.then()).body(path, Matchers.equalTo(matchers), new Object[0]);
        }
        ...
        System.out.println("\n\nEntries differing\n--------------------------");
        difference.entriesDiffering().forEach((key, value) -> {
        EnvGlobals.Differnce.append(key + ": " + value);
        });
        System.out.println("\n\nEntries in common\n--------------------------");
